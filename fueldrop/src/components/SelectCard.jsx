...
function SelectCard({ item, isSelected, onToggleSelect, onRemove  }){
    ...
    <div className="selectCard"
            style={{
                border: isSelected ? "2px solid #355872" : "1px solid #ccc",
                padding: "1rem",
                marginBottom: "1rem"
            }}
        >
        <div className="selectCardHeader">
           <h6> Reg/Serial No. - {item.regNumber} </h6>
        </div>
        
        <div className="SelectCardBody">
            <h5 className="card-title">{item.make} - {item.model}</h5>
            <h5 className="card-title">{item.type}</h5>
            <p className="card-text">{item.fuel}  </p>
            <p className="card-text">Max fuel capacity - {item.litres}L </p>
            <Button
                type = "button"
